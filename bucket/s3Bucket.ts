import { S3Client, DeleteObjectCommand, GetObjectCommand, PutObjectAclCommand } from '@aws-sdk/client-s3'
...
function getSignedUrl(operation: 'get' | 'put', key: string, bucketId: string) {
    const Command = operation === 'get' ? GetObjectCommand : PutObjectAclCommand;
    return getSignedS3Url(
        s3,
        new Command({ Bucket: bucketId, Key: key }),
        { expiresIn: SIGNED_URL_EXIPRES.as('seconds') }
    )
}
